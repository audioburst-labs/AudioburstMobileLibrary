...
class PlaybackEventHandlerInteractorTest {
    ...
    @Test
    fun testIfAdvertisementEventIsNotGettingSavedWhenThereIsAnNoErrorWhileSendingIt() = runTest {
        // GIVEN
        val playbackEvent = PlaybackEvent.AdListened(
            advertisement = advertisementOf(),
            reportingData = reportingDataOf(),
            eventPayload = eventPayloadOf()
        )
        val userRepository: UserRepository = userRepositoryOf(
            returns = MockUserRepository.Returns(
                postAdvertisementEvent = Resource.Data(Unit),
            )
        )
        val unsentEventStorage: UnsentEventStorage = InMemoryUnsentEventStorage()
        ...
        // THEN
        assertTrue(unsentEventStorage.getAllAdvertisementEvents().isEmpty())
    }

    @Test
    fun testIfAdvertisementEventIsGettingSavedWhenThereIsAnErrorWhileSendingIt() = runTest {
        // GIVEN
        val playbackEvent = PlaybackEvent.AdListened(
            advertisement = advertisementOf(),
            reportingData = reportingDataOf(),
        ...
        val userRepository: UserRepository = userRepositoryOf(
            returns = MockUserRepository.Returns(
                postAdvertisementEvent = resourceErrorOf(),
            )
        )
        val unsentEventStorage: UnsentEventStorage = InMemoryUnsentEventStorage()
        ...
        // THEN
        assertTrue(unsentEventStorage.getAllAdvertisementEvents().isNotEmpty())
    }
}
